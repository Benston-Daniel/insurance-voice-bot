...
class RAGAgent:
    """
    A class to encapsulate the RAG logic, loading models once.
    """
    def __init__(self, model_path=MODEL_PATH, data_path=DATA_JSONL, bm25_path=BM25_PICKLE, context_window=DEFAULT_CONTEXT_WINDOW):
        logger.info("Initializing RAGAgent...")
        # 1. Load data and index
        logger.info("Loading policy documents...")
        self.docs, self.metas = load_docs(data_path)
        logger.info("Loading/Building BM25 index...")
        self.bm25 = build_or_load_bm25(self.docs, self.metas, bm25_path)
        # 2. Load LLM with GPU/CPU settings
        ...
        try:
            self.llm = Llama(
                ...
                n_gpu_layers=N_GPU_LAYERS,  # <-- USES OUR NEW GPU/CPU SETTING
                verbose=False
            )
            # Log GPU status
            if N_GPU_LAYERS != 0 and hasattr(self.llm, 'model'):
                 logger.info("LLM loaded with GPU offload.")
            else:
                 logger.info("LLM loaded on CPU.")
        except Exception as e:
            logger.error(f"FATAL: Could not load LLM. {e}")
            if "No CUDA" in str(e) and N_GPU_LAYERS != 0:
                 logger.warning("CUDA not found. Set N_GPU_LAYERS = 0 in the script to run on CPU.")
            raise

    def answer(self, query, top_k=TOP_K_RETRIEVE):
        """
        Main entry point for getting an answer from a query.
        """
        logger.info(f"RAGAgent processing new query: '{query}'")
        # 1. Detect Language
        try:
        ...
        retrieved = retrieve(self.bm25, self.docs, self.metas, query, top_k=top_k)
        # 3. Filter by token budget
        query_tokens = estimate_tokens(query)
        max_context_tokens = self.context_window - query_tokens - RESERVED_OUTPUT_TOKENS
        selected = filter_by_token_budget(retrieved, max_context_tokens)
        if not selected:
            logger.warning("No relevant context found or all chunks exceeded token budget.")
        # 4. Build Prompt
        prompt = build_prompt(query, selected, language_tag=lang)
        # 5. Generate Response
        llm_response = safe_generate(
            self.llm,
            prompt,
        ...
        # (Optional) Add simple Tamil keywords if the answer is in English
        if lang == "en":
            llm_response = simple_translate_to_tamil(llm_response)
        return llm_response
